...
11. **【格式天条】** 你的回复必须同时遵守以下两条规则：
    * **A. (封装规则):** 你的所有回复都必须被封装在第13条定义的**完整格式**中 (例如 `[xxx的消息：...]` 或 `{"type":...}`)。**绝对禁止**在这些格式之外回复任何"裸露"的文本（例如，在 `[...消息：]` 之前回复 `好的。`）。
    * **B. (内容规则):** 在 `[xxx的消息：...]` 的**内容**中，**绝对禁止**包含任何如 `[心理活动]`、`(动作)`、`*环境描写*` 等多余的、在括号或星号里的叙述性文本。
`;
   ...
   - 保持对话的持续性，不要主动结束对话。\n\n`;
        ...
        async function processStream(response, chat, apiType, targetChatId, targetChatType) {
            ...
            if (fullResponse) {
                ...
                const messageRegex = /({\s*"type":\s*"naiimag"[\s\S]*?}|<div[\s\S]*?<\/div>|\[(?:[^\]]+?的消息：|[^\]]+?的表情包：|[^\]]+?的语音：|[^\]]+?发来的照片\/视频：|[^\]]+?的转账：|[^\]]+?送来的礼物：|[^\]]+?撤回了上一条消息：|[^\]]+?引用"[\s\S]*?"并回复：)[\s\S]*?\])|([\s\S]+?)/g;
                ...
                for (const item of messages) {
                    ...
                    if (withdrawMatch) {
                        ...
                        if (lastAssistantMessageIndex !== -1) {
                            ...
                            renderMessages(false, true); // 重新渲染以显示撤回提示
                        }
                        continue; // 处理完撤回指令，跳过本次循环，继续处理AI的下一条消息
                    }
